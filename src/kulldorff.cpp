...
NumericVector multinom(int size, NumericVector probs) {
  ...
  rmultinom(size, REAL(probs), k, &INTEGER(ans)[0]);
  UNPROTECT(1);
  return(ans);
}
...
// [[Rcpp::export]]
NumericVector kulldorffMC2(int nSims,
  NumericVector expectedCases, 
  List nearestNeighborsList, 
  String logLkhdType) {
  ...
  NumericVector allLogLkhd(nZones);
  NumericVector permutedCases(nAreas); 
  NumericVector probs(nAreas); 
  NumericVector maxLogLkhd(nSims);
  
  int total = 0;
  double totalCases = sum(expectedCases);
  total = (int) ceil(totalCases);
  for(int i = 0; i < nAreas; ++i) {
    probs[i] = expectedCases[i]/totalCases;
  }
  ...
  */
  for(int j=0; j < nSims; ++j) {
    permutedCases = multinom(total, probs);
