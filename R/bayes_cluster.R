...
#' @author Albert Y. Kim  
#' @examples  
#' Note for the NYleukemia example, 4 census tracts were completely 
#' surrounded by another unique census tract; when applying the Bayesian cluster detection 
#' model in \code{\link{bayes_cluster}}, we merge them with the surrounding census tracts yielding \code{n=277} areas.
#' 
...
#' 
#' Identify the 4 census tract 
#' to be merged into their surrounding census tracts 
#' 
...
#' 
#' sp.obj <-
#' SpatialPolygons(sp.obj@polygons[-remove],
...
cutoffs <- list(low=RR.range[min(which(pN/pW>1))],high=RR.range[max(which(pN/pW>1))])
# -------------------------------------------------------------------------------
