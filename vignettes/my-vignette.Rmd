...
```{r, message=FALSE, collapse=TRUE, tidy=TRUE, cache=TRUE}
library(spdep)
kappaval <- function(Y, fitted, df) {
    sum((Y - fitted)^2/fitted)/df
}
mod <- glm(Y ~ 1, offset = log(E), family = "quasipoisson", data=nc.sids)
kappaest <- kappaval(nc.sids$Y, mod$fitted, mod$df.resid)
nMC <- 1000
ncts <- length(nc.sids$E)
yMC <- matrix(rpois(n = nMC * ncts, lambda = nc.sids$E), nrow = ncts, ncol = nMC)
kappaMC <- NULL
for (i in 1:nMC) {
    modMC <- glm(yMC[, i] ~ 1, offset = log(E), family = "quasipoisson", data=nc.sids)
...
```{r, message=FALSE, collapse=TRUE, tidy=TRUE}
library(INLA)
nc.sids$ID <- 1:100
m0 <- inla(
  formula = SID74~f(ID, model="iid"),
  family="poisson", 
  E=EXP74,
  data=nc.sids@data,
...
We now compare the of posterior means
...
clustering of cases inneighboring areas.
...
Moran's test may suggest spatial autocorrelation if there exists a non-constant mean function. Below we fit a model with Eastings and Northings (of the County seat) as covariates -- both show some association and the significance of the Moran statistic is reduced, though still significant.

```{r, message=FALSE, collapse=TRUE, tidy=TRUE,tidy.opts=list(width.cutoff=50)}
quasipmod2 <- glm(
  formula = SID74~east+north,
  offset=log(EXP74),
  data=nc.sids@data,
  family=quasipoisson())
summary(quasipmod2)
sidsres2 <- residuals(quasipmod2, type="pearson")
nc.sids$res <- sidsres2
```
...
We now use Moran's statistic on the detrended residuals, but with the binary ``B" weight option. This option has unstandardized weights. Note the asymmetry in the ``W" weights option in the figure below. The conclusion, evidence of spatial autocorrelation, is the same as with the standardized weights option.
...
We implement Openshaw's method using the centroids of the areas in data. Circles of radius 30 are used and the centers are placed on a grid of size 10. For multiple radii, multiple calls are required. The significance level for calling a cluster is 0.002.
...
nc.sids$cases <- nc.sids$SID74
# superfluous
nc.sids$E <- nc.sids$BIR74*referencep
nc.sids$SMR <- nc.sids$cases/nc.sids$E
n <- length(nc.sids$cases)
```
